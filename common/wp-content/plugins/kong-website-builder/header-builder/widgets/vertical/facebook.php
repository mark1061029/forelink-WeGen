...
add_shortcode('lhb_v_facebook', 'lhb_v_facebook_callback');
function lhb_v_facebook_callback($attrs) {
    ...
    if ($attrs['page']) :
        $params = 'href=' . urlencode($attrs['page']) . '&adapt_container_width=false&width=' . $attrs['width'] . '&height=' . $attrs['height'];
        ...
        ob_start(); ?>
        <iframe src="https://www.facebook.com/plugins/page.php?<?php echo $params; ?>&appId" width="100%" height="<?php echo $attrs['height']; ?>" style="border:none;overflow:hidden" scrolling="no" frameborder="0" allowTransparency="true"></iframe>
        <?php
        $html = ob_get_clean();
    endif;

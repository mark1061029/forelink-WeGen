...
ob_start();
?>
<div class="kong-dnd__emptyItem kong-dnd__emptyItem--v" style="height:100px;" ng-if="!$ctrl.edited.attrs.img.url">
    <span class="kong-dnd__emptyItem__label">Logo</span>
</div>

<a class="kong-aside__logo {{::$ctrl.getID()}}" ng-class="[$ctrl.edited.attrs.hidden, $ctrl.edited.attrs.class]"></a>
<?php
$widget['visual_view'] = ob_get_clean();
$manager->add($widget);


add_shortcode('lhb_v_logo', 'lhb_v_logo_callback');

function lhb_v_logo_callback($attrs) {
    ...
    if ($link) {
        ...
        $before_link = '<a href="' . $link . '" class="kong-aside__logo ' . implode(' ', $classes) . '"' . $target . '>';
        $after_link = '</a>';
    } else {
        $before_link = '<span class="kong-aside__logo ' . implode(' ', $classes) . '">';
        $after_link = '</span>';
    }
